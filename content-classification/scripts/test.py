...
def test(input_shape, classes, dataset_dir, model_dir, model, out_dir, batch_size=256):
    np.random.seed(1337)
    random.seed(1337)
    ...
    # evaluate
    results = model.evaluate(test_iter, batch_size=batch_size, verbose=2)
...
if __name__ == '__main__':
    ...
    test_loss, test_accuracy = test((args.input_shape, args.input_shape, 3), classes, os.path.join(args.data_dir, 'test'), args.model_dir,
                                    None, args.out_dir, args.batch_size)
